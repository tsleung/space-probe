extends RefCounted
class_name ColonySimAI

## Colony Sim AI Controller
...
static func _score_choice(choice: Dictionary, state: Dictionary, personality: Personality, random_value: float) -> float:
	...
	var pop_count = ColonySimPopulation.count_alive(state.colonists)
	var stability = state.politics.stability
	var food = state.resources.get("food", 0.0)
	...
	for resource_name in resource_changes:
		...
		var current = state.resources.get(resource_name, 100.0)
...
static func choose_building(state: Dictionary, personality: Personality, random_value: float) -> int:
	...
	var pop_count = ColonySimPopulation.count_alive(colonists)
	var workforce = ColonySimPopulation.get_workforce_summary(colonists)

	# Calculate current capacities
	var housing_balance = ColonySimEconomy.calc_housing_balance(buildings, colonists)
	var power_balance = ColonySimEconomy.calc_power_balance(buildings, colonists)
	...
	for b in buildings:
		if b.type == ColonySimTypes.BuildingType.MEDICAL_BAY:
			has_medical = true
			break
	if not has_medical and pop_count > 30:
		priorities.append({
			"type": ColonySimTypes.BuildingType.MEDICAL_BAY,
			"priority": 70
		})
	...
	for b in buildings:
		if b.type == ColonySimTypes.BuildingType.SCHOOL:
			has_school = true
			break
	var child_count = 0
	for c in colonists:
		if c.is_alive and c.life_stage == ColonySimTypes.LifeStage.CHILD:
			child_count += 1
	if not has_school and child_count >= 3:
		priorities.append({
			"type": ColonySimTypes.BuildingType.SCHOOL,
			"priority": 60
		})
	...
	match personality:
		Personality.VISIONARY:
			...
			priorities.append({
				"type": ColonySimTypes.BuildingType.LAB,
				"priority": 50 + random_value * 30
			})
		Personality.HUMANIST:
			# Boost comfort buildings
			priorities.append({
				"type": ColonySimTypes.BuildingType.RECREATION_CENTER,
...
static func _get_building_cost(building_type: int) -> Dictionary:
	match building_type:
		...
		ColonySimTypes.BuildingType.LAB:
			return {"building_materials": 90, "machine_parts": 25}
		ColonySimTypes.BuildingType.RECREATION_CENTER:
...
static func run_ai_turn(store: Node, personality: Personality, rng: RandomNumberGenerator) -> Dictionary:
	...
	var active_events = state.active_events
	for event in active_events:
		var choice_idx = choose_event_option(event, state, personality, rng.randf())
		var choice = event.choices[choice_idx] if choice_idx < event.choices.size() else event.choices[0]
		store.resolve_event(event.id, choice_idx)
		actions.append("Event '%s': chose '%s'" % [event.title, choice.text])
	...
	if rng.randf() < 0.3 or _has_critical_need(state):
		...
		if building_type >= 0:
			...
			actions.append("Started construction: %s" % ColonySimTypes.get_building_name(building_type))
...
static func _has_critical_need(state: Dictionary) -> bool:
	...
	var housing = ColonySimEconomy.calc_housing_balance(buildings, colonists)
	if housing.available <= 0:
		return true

	var power = ColonySimEconomy.calc_power_balance(buildings, colonists)
	if power.balance < 0:
		return true

	var food = state.resources.get("food", 0.0)
	var pop = ColonySimPopulation.count_alive(colonists)
