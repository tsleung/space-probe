...
## Colony Sim Event Logic - SIMPLIFIED
...
static func select_yearly_events(state: Dictionary, random_values: Array) -> Dictionary:
	...
	for event in eligible:
		...
		var last_trigger = state.event_cooldowns.get(event.type, 0)
...
static func apply_event_choice(state: Dictionary, event: Dictionary, choice_id: String, random_value: float) -> Dictionary:
	var choice = null
	...
	if effects.has("stability_change"):
		new_state.politics = ColonySimPolitics.update_stability(
			new_state.politics, effects.stability_change
		)

	# Apply resource changes
	if effects.has("resource_changes"):
		var res = new_state.resources.duplicate()
		for resource_name in effects.resource_changes:
			res[resource_name] = maxf(0, res.get(resource_name, 0) + effects.resource_changes[resource_name])
	...
	var outcome = choice.get("outcome", "Choice made: %s" % choice.text)
	if not success:
