...
## Colony Sim Political Logic - SIMPLIFIED
...
static func update_faction_standings(politics: Dictionary, colonists: Array, resources: Dictionary) -> Dictionary:
	...
	if food < 100:
		standings[ColonySimTypes.Faction.PRAGMATISTS] = minf(100, standings.get(ColonySimTypes.Faction.PRAGMATISTS, 50) + 2.0)
	...
	for c in colonists:
		if c.is_alive:
			total += 1
			if c.generation != ColonySimTypes.Generation.EARTH_BORN:
				mars_born += 1

	if total > 0 and float(mars_born) / float(total) > 0.5:
		standings[ColonySimTypes.Faction.MARTIANS] = minf(100, standings.get(ColonySimTypes.Faction.MARTIANS, 50) + 1.0)
