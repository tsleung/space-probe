...
func _draw_battle_ui(rect: Rect2) -> void:
	...
	draw_string(font, Vector2(rect.size.x / 2 - 50, 16), "⚔ %s" % _zone_name.to_upper(),
		HORIZONTAL_ALIGNMENT_CENTER, 100, 10, Color.WHITE)
	...
	draw_string(font, Vector2(8, 16), "UNN: %d" % player_alive, HORIZONTAL_ALIGNMENT_LEFT, -1, 9, Color(0.4, 0.7, 1.0))
	draw_string(font, Vector2(rect.size.x - 60, 16), "HERALD: %d" % herald_alive, HORIZONTAL_ALIGNMENT_LEFT, -1, 9, Color(1.0, 0.35, 0.2))

	var bar_w = 100
	var bar_x = rect.size.x / 2 - bar_w / 2
	var progress = minf(_battle_time / _battle_duration, 1.0)
	draw_rect(Rect2(bar_x, 22, bar_w, 4), Color(0.15, 0.15, 0.15))
	draw_rect(Rect2(bar_x, 22, bar_w * progress, 4), Color(0.7, 0.4, 0.1))
...
func _draw_expand_button(rect: Rect2) -> void:
	...
	# Button background
	var bg_color = Color(0.2, 0.4, 0.6, 0.9)
	draw_rect(_expand_button_rect, bg_color)
	draw_rect(_expand_button_rect, Color(0.4, 0.6, 0.8, 0.9), false, 1)

	# Icon: expand (arrows pointing out) or collapse (arrows pointing in)
	var icon = "◱" if _is_expanded else "◳"
	draw_string(font, Vector2(btn_x + 8, btn_y + 13), icon, HORIZONTAL_ALIGNMENT_CENTER, btn_w, 11, Color.WHITE)
...
func _update_camera(delta: float) -> void:
	...
	_camera_offset = _camera_offset.lerp(_camera_target, delta * 2)
	_camera_zoom = lerpf(_camera_zoom, _target_zoom, delta * 2)

func _update_cinematics(delta: float) -> void:
	# Change camera focus periodically
	if int(_cinematic_timer) % 4 == 0 and int(_cinematic_timer - delta) % 4 != 0:
		# Pick a random ship to follow
...
func start_battle(zone_name: String, player_ships: Array, herald_count: int, will_hold: bool) -> void:
	...
	for i in range(mini(player_ships.size(), 4)):
		var ship = BattleShip.new()
		ship.ship_data = player_ships[i]
		ship.pos = Vector2(size.x * 0.75, center.y + (i - 1.5) * 40)
		ship.rotation = PI
		ship.size = 12 + (i % 2) * 4
		ship.is_player = true
		ship.health = 1.0
	...
	for i in range(mini(herald_count, 6)):
		var ship = BattleShip.new()
		ship.pos = Vector2(size.x * 0.2 + randf() * 40, 40 + i * ((size.y - 80) / max(herald_count - 1, 1)))
		ship.rotation = 0
		ship.size = randf_range(10, 16)
